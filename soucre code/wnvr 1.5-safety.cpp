...
int main()
{
	if (MessageBoxW(NULL, L"Run wnvr 1.5.exe safety", L"wnvr(1.5)-safety.exe by VietNamLover and NguoiVietNam01", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure??!!", L"wnvr(1.5)-safety.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
