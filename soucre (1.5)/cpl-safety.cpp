...
VOID WINAPI sound1() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * 3 & -t >> 7) % 77);
...
VOID WINAPI sound2() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t >> 7 | t >> 10) & 30));
...
VOID WINAPI sound5() {
	...
	char buffer[16000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * ((t >> 6 | t >> 12) & 14 & t >> 9)) * t >> 9);
...
VOID WINAPI sound7() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(((t * (t >> 8 | t >> 3) | t >> 1) ^ t) + 64);
...
VOID WINAPI sound8() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (1000 >> (t >> 9 & 14) & 14) | t >> 9);
...
VOID WINAPI sound9() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t & 4096 ? t % 65536 < 59392 ? 7 : t & 7 : 16) + (1 & t >> 14)) >> (3 & -t >> (t & 2048 ? 2 : 10)) | t >> (t & 1637 ? t & 4096 ? 10 : 2 : 2));
...
VOID WINAPI sound10() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t >> 8 | t >> 8) & 31) & t << 2);
...
int main()
{
	if (MessageBoxW(NULL, L"cpl.exe, GDI only, Run?", L"cpl.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure", L"cpl.exe - Final Warning!", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
			...
			InvalidateRect(0, 0, 0);
			Sleep(100);
			HANDLE thread4 = CreateThread(0, 0, pixelate, 0, 0, 0);
			HANDLE thread4dot1 = CreateThread(0, 0, balls, 0, 0, 0);
			sound4();
			Sleep(30000);
